...
def find_highest_number(
    sequence: str, n=0, collection=None, target=2, og_sequence=None
) -> int:
    ...
    if n == target:
        ...
        n += 1
        collection[n] = {"number": highest_number, "index": og_number_index}

        print(f"Found highest number: {highest_number}")
        if len(sequence[highest_number_index:]) > 0:
            print(
                f"Fowardly checking the rest of the sequence: {sequence[highest_number_index:]}"
            )
            return find_highest_number(
                sequence[highest_number_index:], n, collection, target, og_sequence
            )
        else:
            print(
                f"Backwardly checking the rest of the sequence: {sequence[: highest_number_index - 1]}"
            )
            return find_highest_number(
                sequence[: highest_number_index - 1], n, collection, target, og_sequence
            )


def sliding_window(sequence: str, window_size: int) -> list:
    return [
        sequence[i : i + window_size] for i in range(len(sequence) - window_size + 1)
    ]
